                ...
                <div className=" max-sm:pb-5 scroller_none max_width_layout w-full flex max-sm:overflow-x-scroll md:grid grid-cols-1 md:grid-cols-6 gap-4 md:gap-8 mt-8 md:mt-12">
                    {services.map((service, i) => (
                        <div key={i} className={` group shrink-0 relative overflow-hidden w-[90%] md:w-full h-[45vh] md:h-[50vh] group hover:text-white transition-colors duration-300 bg-white p-10 rounded-2xl ${service.className}`}>
                            {service.img && <img src={service.img} className='absolute group-hover:opacity-100 brightness-90 transition-opacity duration-300 opacity-0  inset-0 cover' alt="loading" />}
                            <div className=" relative h-full w-full flex flex-col justify-between">
                                <div className="w-24 relative center">
                                    <img src={service.unfill_icon} className=' opacity-0 w-full' alt="loading" />
                                    <img src={service.unfill_icon} className=' opacity-100 group-hover:opacity-0 transition-opacity duration-300  absolute w-full' alt="loading" />
                                    <img src={service.fill_icon} className=' opacity-0 group-hover:opacity-100 transition-opacity duration-300  absolute w-full' alt="loading" />
                                </div>
                                <div className="">
